...
本节把 `questions.full.clean.md` 与既有真题映射中的高频考法反查回本专题。2019 下午题要求计算 PV/EV/AC/CV/SV/CPI/SPI；上午题也会考只有 PV 和 AC 时无法判断绩效。 学习时不要只背定义，要能从题干信号判断它在考哪一个管理动作或技术边界。
...
## Anki 补强：EVM 公式链和待核验隔离

挣值管理制卡应形成“PV/EV/AC 概念 → SV/CV/SPI/CPI 公式 → EAC/ETC/VAC/TCPI 预测 → 信息不足陷阱 → 下午案例表达”的链条。

其中 `EAC = AC + (BAC - EV) / (CPI × SPI)` 属于扩展口径。若教材原文未明确要求同时考虑成本和进度影响，应放入 `needs_human_review`，不要和高频的非典型偏差、典型偏差 EAC 公式混在主复习里。

计算卡必须保留文字判断，例如“进度滞后且成本超支”，不能只写数字。

> **例题｜EVM 数据是否充分（真题型改编训练题）**
>
> 某项目第三个月累计计划费用 PV 为 25 万元，实际成本 AC 为 28 万元，但题目没有给出 EV。以下判断最稳妥的是：
>
> A. 成本一定超支  
> B. 进度一定提前  
> C. 信息不足，无法判断成本和进度绩效  
...
**题干关键词：** PV、AC、没有 EV、无法评估。

**解题过程：** EVM 判断成本和进度绩效必须知道 EV。只有 PV 和 AC，不能知道已经完成了多少价值。
...
- A 成本是否超支要看 EV-AC。
- B 进度是否提前要看 EV-PV。
- D 是否完工超预算还需要 EAC 等预测。

**举一反三：** 挣值题先列三值：PV 是计划值，EV 是挣得值，AC 是实际成本；缺 EV 时很多结论都不能下。
