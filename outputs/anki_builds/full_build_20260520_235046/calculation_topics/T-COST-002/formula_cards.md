...
## 同时考虑 CPI 和 SPI 的 EAC 扩展公式是什么？

Back：EAC = AC + (BAC - EV) / (CPI × SPI)。

Extra：<div>理解：用于题干明确要求同时考虑成本和进度影响。本专题将该口径列为需回查教材的待核验项，制卡时应保留提醒。</div><div>来源：T-COST-002 试制黄金样例 / 挣值管理专题</div>

Manual review：涉及教材口径待核验的扩展公式，隔离给人工确认。
